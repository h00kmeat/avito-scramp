...
import asyncio
from handlers.updater import process_updating
router = Router()
...
async def cmd_start(message: types.Message, db_pool, engine, state: FSMContext):
    ...
    key.setdefault("current_user_id", user_id)
    ...
    await state.update_data(updating_active=True)
    
    asyncio.get_running_loop().create_task(process_updating(user_id,engine))
